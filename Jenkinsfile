        ...
        stage('Deploy to swarm cluster'){
            steps {                  
                 sshagent(['docker-swarm-master']){                    
                     ...
                     sh "ssh -o StrictHostKeyChecking=no -t ec2-user@52.66.172.85  'cd /usr/local/bin/docker-swarm-workloads ; bash | docker stack deploy -c docker-compose.yml docker-restapp-stack'"
                 }
            }
            post {
                success {
